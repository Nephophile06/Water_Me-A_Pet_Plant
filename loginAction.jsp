...
<%
    ...
    String hashedPassword = hashPassword(pass.trim());

    try (Connection conn = DBConnection.getConnection()) {
        if ("register".equals(action)) {
            String checkSql = "SELECT user_id FROM plant_users WHERE username = ?";
            PreparedStatement checkPs = conn.prepareStatement(checkSql);
            checkPs.setString(1, user.trim());
            ResultSet rs = checkPs.executeQuery();

            if (rs.next()) {
                response.sendRedirect("auth.jsp?error=user_exists");
                return;
            }

            String insertSql = "INSERT INTO plant_users (username, password_hash) VALUES (?, ?)";
            PreparedStatement insertPs = conn.prepareStatement(insertSql, new String[]{"USER_ID"});
            insertPs.setString(1, user.trim());
            insertPs.setString(2, hashedPassword);
            insertPs.executeUpdate();

            ResultSet genKeys = insertPs.getGeneratedKeys();
            int newUserId = 0;
            if (genKeys.next()) newUserId = genKeys.getInt(1);
